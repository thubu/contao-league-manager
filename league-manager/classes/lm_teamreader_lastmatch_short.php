 ...
 * @package    Controller
 */
class lm_teamreader_lastmatch_short extends ContentElement
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'lm_teamreader_lastmatch_short';

	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			$return="Last match per team - shortname<br />";
			if($this->lm_usefixedteam=="1"){
				$objTeam =$this->Database->prepare("SELECT * FROM tl_lm_teams WHERE id=?")->execute($this->lm_team);
				$return.="Fixed team: " . $objTeam->shortname;
			}
			else
			{
			...
			if($this->lm_usefixedcontest=="1"){
							...
							$return.="Fixed contest: " . $objContest->shortname . " <br />";
						}
						else
						{
	...
	protected function compile()
	{
		switch($this->lm_template){
				case "div":
					$objTemplate = new FrontendTemplate("lm_teamreader_lastmatch_short");
					break;
				case "table":
					$objTemplate = new FrontendTemplate("lm_teamreader_lastmatch_short_table");
					break;
				default:
					$objTemplate = new FrontendTemplate("lm_teamreader_lastmatch_short");
			}
		...
		if($teamid)
		{
			...
			if($objMatch->numRows>0)
			{
				...
				$this->Template->home_shortname=$home->shortname;
				...
				$this->Template->away_shortname=$away->shortname;
				if($objMatch->venue == 'H'){
				...
				$this->Template->away_name=$away->shortname;
