							...
							<?php foreach($this->matches[$i] as $match): ?>
								<div class="match<?php if($match['home_own']==1||$match['away_own']==1): ?> own<?php endif; ?>">
									...
									<div class="teams">
										...
										<?php if ($this->show_logo==1 && $this->home_logo!=''): ?><?php if (!is_numeric($this->home_logo))
										//convert DB file ID into file path ($objFile->path) 
										$objFile = \FilesModel::findByPk($this->home_logo);$path=$objFile->path;?><img src="<?php echo \FilesModel::findByPk($this->home_logo)->path; ?>">
										<?php endif; ?> 
									...
									<div class="result">
										...
										<span class="halftimescorehome<?php if($match['home_own']==1): ?> own<?php endif; ?>"><?php echo $match['homescorehalftime']; ?></span>
										<span class="versus">:</span>
										<span class="halftimescoreaway<?php if($match['away_own']==1): ?> own<?php endif; ?>"><?php echo $match['awayscorehalftime']; ?></span>
